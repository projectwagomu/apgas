 ...
 * for their specific program using {@link ExtendedConstructs#defineMalleableHandle(MalleableHandler)}
...
public interface MalleableHandler extends Serializable {
	...
	/**
	 * Method called when a shrink order is received by the scheduler.
	 * In this method, all the preparations necessary to the program prior to 
	 * the actual release of places should be performed before returning a list
	 * containing the list of places to release. The places returned in the list
	 * should be distinct and match the number of places indicated by the 
	 ...
	 * running program.
	 * If any preparation is needed prior to the increase in the number of
	 ...
	 * @param nbPlaces number of places that will be added to the running
	 * program
	 */
